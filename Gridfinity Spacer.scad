...
width_shift_offset=((available_space_width - required_grid_space_width - (base_plate_corners_rounded==true ? gridfinity_corner_diameter : 0)) / 2) - width_offset;
length_shift_offset=((available_space_length - required_grid_space_length - (base_plate_corners_rounded==true ? gridfinity_corner_diameter : 0)) / 2) - length_offset;

final_width_shift=width_alignment=="left" ? width_shift_offset * -1 : width_alignment=="right" ? width_shift_offset : 0;
final_length_shift=length_alignment=="front" ? length_shift_offset * -1 : length_alignment=="back" ? length_shift_offset : 0;
...
linear_extrude(spacer_height) {
    difference() {
        ...
        translate([final_width_shift, final_length_shift, 0]) {
            minkowski() {
                if (base_plate_corners_rounded) circle(d=gridfinity_corner_diameter);
                square([required_grid_space_width, required_grid_space_length], true);
            }
        }
    }
}
